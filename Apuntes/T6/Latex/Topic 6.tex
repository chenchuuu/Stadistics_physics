...
\begin{document}
	...
	\chapter{Macrocanonical Ensemble}
	...
	\begin{equation}
		N_A + N_B = \text{const} \quad \rightarrow \quad dN_A + dN_B = 0 \tag{6.1}
	\end{equation}
	...
	\begin{align}
		\mu_A + \mu_B > \mu_C \quad &\implies \quad \text{A + B} \rightarrow \text{C} \tag{6.6} \\
		\mu_A + \mu_B < \mu_C \quad &\implies \quad \text{C} \rightarrow \text{A + B} \tag{6.7}
	\end{align}
	...
	In Fig. 6.3, we schematically show a representative member $S$ of the macrocanonical ensemble that exchange energy and mass with its surrounding reservoir $W$. The net mass exchange is zero at the thermodynamic equilibrium, when $T_s = T_w$ and $\mu_s = \mu_w$. 
	...
	\noindent Additionally, the numerical density is basically constant across all $U$:
	...
	\textbf{Figure 6.4:} Sketch of a macroscopic system $S$ exchanging mass and energy with its surroundings $W$.
	...
	where $\Xi = \Xi(\mu, T, V)$ is the \textbf{macrocanonical partition function} and $P_{mN}$ is the \textbf{macrocanonical probability}. Note that both $\Xi$ and $P_{mN}$ do not depend on $W$.
